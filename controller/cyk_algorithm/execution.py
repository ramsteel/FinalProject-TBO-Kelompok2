...
import controller.cyk_algorithm.table as table
...
def exec(newDct, fillTable, inputString):
    cnt = 0
    global dct
    dct = newDct
    strTable = copy.deepcopy(fillTable) # copy tabel fillTable dijadikan ke dalam bentuk string
    lstInput = inputString.split(' ')    # memisah input ke dalam list
    for x in range(len(lstInput)):
        ...
        for j in range(x,len(lstInput)):
            ...
            if fillTable[i][j] == set() and cnt == 0:
                print('Maaf, terdapat kata yang tidak sesuai pada rules')
                return strTable, -1 
    ...
    # Mencetak isi di dalamnya
    table.printTable(strTable)
...
def isAccepted(fillTable):
    ...
    if list(dct.keys())[0] in fillTable[0][len(fillTable)-1]: 
        print('Kalimat Valid\n') #string diterima
        st.write('Kalimat Valid')
        return True
    else: 
        print('Kalimat Tidak Valid\n') #jika tidak string ditolak
        st.write('Kalimat Tidak Valid')
        return False
